...
import { restoreDomainFn, deleteDomainFn } from "@/api/domainApi";
...
const Index = () => {
  ...
  } = useMutation(({ id }) => restoreDomainFn({ id }), {
    onSuccess: ({ message }) => {
  ...
  const {
    ...
    mutate: deleteDomain,
  } = useMutation(({ id }) => deleteDomainFn({ id }), {
    onSuccess: ({ message }) => {
      toast.success(message);
      queryClient.invalidateQueries(["trashed-domains-count"]);
    },
  ...
  useUpdateEffect(() => {
    ...
    $("#domain_trash_index").DataTable({
      ajax: {
        url: "https://lets-comply-backend.auguma.io/admin/domains/trashed",
        headers: {
            ...
            createRoot(td).render(
              <div className="flex">
                <button
                  ...
                  onClick={() => restoreDomain({ id: rowData.id })}
                >
                ...
                <button
                  ...
                  onClick={() => deleteDomain({ id: rowData.id })}
                >
                  <span className="w-4 inline-flex align-middle">
                    <DeleteIcon />
                  </span>
  ...
  useUpdateEffect(() => {
    //initialize datatable
    $("#domain_trash_index").DataTable().ajax.reload();
  }, [isSuccess, isDeletedSuccess]);
  return (
    <table id="domain_trash_index" className="display compact pt-3">
