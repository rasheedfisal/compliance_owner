...
const Show = ({ params: { DId } }: PageProps) => {
  ...
  const { isLoading: isInvitationLoading, data } = useQuery(
    ["invitations", DId],
    () => getInvitationFn(DId),
    {
      select: (data) => data,
  ...
  const { isLoading, mutate: deleteInvitation } = useMutation(
    ({ id }: { id: string }) => deleteInvitationFn({ id }),
    {
      onSuccess: ({ message }) => {
        toast.success(message);
        router.push("/initations");
      },
      onError: (error: any) => {
        if ((error as any).response?.data?.message) {
          toast.error((error as any).response?.data?.message, {
