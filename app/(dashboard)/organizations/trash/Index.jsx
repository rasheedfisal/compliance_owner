...
const Index = () => {
  ...
  const {
    ...
    isSuccess,
    mutate: restoreOrg,
  } = useMutation(({ id }) => restoreOrganizationFn({ id }), {
    onSuccess: ({ message }) => {
  ...
  const {
    ...
    mutate: deleteOrg,
  } = useMutation(({ id }) => deleteOrganizationFn({ id }), {
    onSuccess: ({ message }) => {
      toast.success(message);
      queryClient.invalidateQueries(["trashed-org-count"]);
    },
            ...
            createRoot(td).render(
              <div className="flex">
                <button
                  ...
                  onClick={() => restoreOrg({ id: rowData.id })}
                >
                ...
                <button
                  ...
                  onClick={() => deleteOrg({ id: rowData.id })}
                >
                  <span className="w-4 inline-flex align-middle">
                    <DeleteIcon />
                  </span>
  ...
  useUpdateEffect(() => {
    //initialize datatable
    $("#org_trash_index").DataTable().ajax.reload();
  }, [isSuccess, isDeletedSuccess]);
  return (
    <table id="org_trash_index" className="display compact pt-3">
